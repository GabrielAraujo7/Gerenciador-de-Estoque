...
app.get('/buscar-produto', async (req, res) => {
  ...
  if (!nome_produto) {
    return res.status(400).json({ error: 'O parâmetro "nome_produto" é obrigatório' });
  }
...
async function adicionarVenda(nome_cliente, telefone, valor_total, metodo_pagamento, tipo_cliente, taxa_entrega, produtosSelecionados) {
  const data_venda = new Date().toISOString().slice(0, 10);
  const trx = await knex.transaction();

  try {
    // Adicionar cliente
    const cliente_id = await adicionarCliente(nome_cliente, telefone, tipo_cliente);
    ...
    const itemVendaValues = produtosSelecionados.map(produto => {
      ...
      const quantidade = parseInt(produto.quantidadeSelecionada, 10);

      if (isNaN(preco) || isNaN(custo) || isNaN(quantidade)) {
        throw new Error(`Dados inválidos para o produto ${produto.Nome_Produto}`);
      }
...
app.post('/registrar-venda', async (req, res) => {
  const { nome_cliente, telefone, metodo_pagamento, tipo_cliente, taxa_entrega, valor_total, produtos } = req.body;

  if (!nome_cliente || !telefone || !valor_total || !metodo_pagamento || !tipo_cliente || !produtos || produtos.length === 0) {
    return res.status(400).json({ error: 'Todos os campos são obrigatórios, incluindo os produtos' });
  }
  ...
  try {
    ...
    res.status(500).json({ error: 'Erro ao registrar venda', message: error.message });
  }
});
...
app.get('/buscar-vendas', async (req, res) => {
  try {
    const vendas = await knex('venda')
      .select('Venda_ID', 'Nome_cliente', 'Cliente_ID', knex.raw('DATE(Data_Venda) as Data_Venda'), 'Valor_Total', 'Lucro_Produto', 'Metodo_Pagamento', 'Taxa_Entrega', 'Tipo_Cliente')
